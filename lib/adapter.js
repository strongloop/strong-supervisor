...
function writeConfigProperties() {
  ...
  endpoints.forEach(function(endpoint) {
    var uri;
    try {
      // Skip the appmetrics/am- endpoints, they aren't statsd based.
      if (!(uri = url.parse(endpoint)).protocol.match(/^am-/))
        return;
    } catch (er) {
    ...
    var properties = [
      ...
      'com.ibm.diagnostics.healthcenter.mqtt.broker.port=' +
        uri.port || 1883,
// This is not good, we should probably find which of the 3 appmetrics.property
// files they are using, and append the three lines above to it, assuming
// last lines win. But we don't want to modify the one in our node_modules,
// maybe we modify one in their "application" directory (what is that?)
// if it exists, or create/modify one in the CWD whether it exists or not.
// Sucky part of this is its sticky.. run again with no metrics, and the file
// is still there :-(
    ...
    try {
      fs.writeFileSync('appmetrics.properties', properties);
    } catch (er) {
      console.error('Failed to configure `%s`: %s', endpoint, er.message);
      process.exit(1);
    }
  });
}
