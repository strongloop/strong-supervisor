...
var capabilities = {
  watchdog: {
    ...
    checks: [
      ...
      {
        check: function(cb) {
          if (agent().internal.supports.watchdog) { // XXX(sam) appmetrics?
            cb(true);
          } else {
      ...
      {
        check: function(cb) {
          var addon = require('strong-agent/lib/addon'); // XXX(sam) appmetrics?
          if (addon) {
            cb(true);
          } else {
  ...
  tracing: {
    name: 'Tracing',
    checks: [ // XXX(sam) appmetrics never supports this
      {
        check: function(cb) {
  ...
  patch: {
    ...
    checks: [
      {
        check: function(cb) {
          if (agent().dyninst.metrics) { // XXX(sam) appmetrics?
            cb(true);
          } else {
            cb(false,
