...
// Ensure cluster picks up all our changes to process.argv
...
config.start = function start() {
  if(options.detach) {
    ...
    try {
      ...
      var superLog = generateLogName(options.log, { id: 'supervisor',
                                                    pid: process.pid });
      var child = detach(options.argv.concat(options.args), superLog);
      console.error('supervisor %d detached process %d, output logged to \'%s\'',
        process.pid, child.pid, superLog);
