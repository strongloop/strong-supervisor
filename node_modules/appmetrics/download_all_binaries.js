...
var getAgentCorePlatformVersionDownloadURL = function() {
	return [FILE_URL, 'core/tgz'].join('/') + 
	       ['/agentcore', AGENTCORE_VERSION, getPlatform()].join('-') + '.tgz';
};

var getAppMetricsPlatformVersionDownloadURL = function() {
	return [FILE_URL, 'nodejs/tgz', getSupportedNodeVersionOrExit()].join('/') + 
           ['/appmetrics', APPMETRICS_VERSION, getPlatform()].join('-') + '.tgz';
};


var localFileExtractTGZ = function(fileURL, destDir) {
	/* Downloading the binaries */

	fs.createReadStream(fileURL).pipe(zlib.createGunzip()).on('error', function(e) { console.log("Failed to gunzip: " + e.message); })
		        .pipe(tar.Extract({path: destDir})).on('error', function(e) { console.log("Failed to untar: " + e.message); })
		        .on('close', function() {
		        	console.log('Download and extract of ' + fileURL + ' finished.');
		        });
};
...
localFileExtractTGZ(getAgentCorePlatformVersionDownloadURL(), '.');
localFileExtractTGZ(getAppMetricsPlatformVersionDownloadURL(), '.');
